...
public class DatabaseHelper extends OrmLiteSqliteOpenHelper {
  ...
  private void setUpDatabase(SQLiteDatabase db, Integer oldVersion, Integer newVersion) {
    DatabaseConnection con = connectionSource.getSpecialConnection(null);
   ...
   * @param oldVersion
   *         The version of the current database so we can know what to do to the
   *         database.
   * @param newVersion
  ...
  public <D extends Dao<T, ?>, T> D getDao(Class<T> clazz) throws SQLException {
    ...
    if (dao == null) {
      ...
      DatabaseTableConfig<T> tableConfig =
              DatabaseTableConfigUtil.fromClass(connectionSource, clazz);
      if (tableConfig == null) {
        /**
    ...
    @SuppressWarnings("unchecked") D castDao = (D) dao;
    return castDao;
  }
}
